...
Niacin's entire sleep-prevention core today is `Process.run()` against `/usr/bin/caffeinate`. That's been a deliberate "use Apple's tested mechanism" choice and it works fine 99% of the time, but it makes two assumptions worth hardening against.

- **Sleep-prevention integration tests** — currently the test suite exercises `ManagedPreferences`, `availableDurations`, and `ActivationDuration` thoroughly, but `SleepPreventer.swift` itself has zero coverage. Add three tests: (1) argument-construction — verify the right caffeinate flags for each `(duration, allowDisplaySleep)` combo, catches "shipped `-d` when we meant `-di`" regressions; (2) lifecycle — `activate()` → `deactivate()` clears `process` and `isActive`; (3) `pmset -g assertions` introspection — actually verify the running process is holding `PreventUserIdleSystemSleep`. The third one is the one that proves the side-effect, not just the code path. ~60 lines total, macOS-only. (S)
- **Replace caffeinate with direct IOKit `IOPMAssertion` calls** ★ — `caffeinate` is a ~200-line CLI wrapper (Apple's APSL source at `apple-oss-distributions/PowerManagement`) around exactly one IOKit function: `IOPMAssertionCreateWithName`. Going direct from Swift via `IOKit.pwr_mgt` is ~30 lines and replaces the entire `Process.run` dance. Benefits: (a) immune to enterprise EDR/XDR tools (CrowdStrike, SentinelOne, etc.) that block subprocess execution of `/usr/bin/caffeinate`; (b) no `/usr/bin/` path dependency at all; (c) no subprocess lifecycle management / termination handlers / PID tracking; (d) plays cleanly with App Sandbox if that decision ever gets revisited; (e) same kernel primitive — functionally identical, simpler. Map current flags: `-i` → `kIOPMAssertionTypeNoIdleSleep`; `-d` adds `kIOPMAssertionTypePreventUserIdleDisplaySleep`. Niacin's existing duration timer replaces `caffeinate -t`. Drop the caffeinate code path entirely once IOKit is in place — there's no real argument for keeping it as fallback. (M)
- **Detect-and-surface engine failure** — whichever mechanism is primary, when it fails (caffeinate spawn rejected, or `IOPMAssertionCreateWithName` returns non-success), surface it visibly: menu icon shows an error state, tooltip says "Niacin can't prevent sleep — check with IT", and the audit log records the specific failure. Today the user sees nothing and assumes the app is broken. (S)
