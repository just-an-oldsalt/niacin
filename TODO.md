...
- **IOKit `IOPMAssertion` fallback for blocked-caffeinate environments** ★ — some enterprise EDR/XDR tools (CrowdStrike, SentinelOne, etc.) and managed configurations block subprocess execution of `/usr/bin/caffeinate`. When that happens today, Niacin's "Activate" silently does nothing — the menu briefly looks active and then resets, and IT has no diagnostic. Mitigation: hold `IOPMAssertion` directly via IOKit (`IOPMAssertionCreateWithName` with `kIOPMAssertionTypeNoIdleSleep` / `kIOPMAssertionTypePreventUserIdleDisplaySleep`). That's the same primitive `caffeinate` itself uses; eliminates the subprocess dependency entirely. Open question: keep `caffeinate` as primary with IOKit as fallback, or invert it (IOKit primary, caffeinate fallback for parity, or just drop `caffeinate` altogether). IOKit primary is honestly the cleaner architecture — no Process spawning, no `/usr/bin/` dependency, clearer lifecycle, plays better with sandbox if we ever revisit that decision. (M)
